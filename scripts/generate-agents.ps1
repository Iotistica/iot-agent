...
param(
    ...
    [string]$DbHost = "20.220.137.172",
    [int]$DbPort = 5432,
    [string]$DbName = "demo",
    [string]$DbUser = "billing",
    [string]$DbPassword = "2GyDA3MMJvKLwvfo6t@btpmTA9f",
    [string]$DbSslMode = "",
    [string]$DatabaseUrl = "",
...
# This script lives in iot-agent/scripts/, but the generated compose file (and
# its ./certs, ./mosquitto-agent volume mounts) belong in the sibling iotistica
# repo, where docker-compose.agents.yml has always lived and is actually run.
$IotisticaRoot = Join-Path $PSScriptRoot ".." ".." "iotistica"
...
# When building Pro from source, pack the iot-agent-pro package into pro/
...
if ($IsProBuild -and $BuildFromSource) {
    ...
    $proDestDir = Join-Path $PSScriptRoot ".." "pro"
...
function Get-NextAvailableAgentIndex {
    ...
    $outputPath = Join-Path $IotisticaRoot $OutputFile
...
if (
    ...
    $composeFile = Join-Path $IotisticaRoot $OutputFile
...
for ($i = $StartIndex; $i -lt ($StartIndex + $Count); $i++) {
    ...
    $agentShellHmacKeyEnv = '${AGENT_SHELL_HMAC_KEY}'
        ...
        $serviceLines += @(
                ...
                '      # MQTT device-write commands (src/commands/) — off by default in code',
                '      # (see createCommandFeature in src/commands/index.ts); enabled here for',
                '      # this test environment only. Still requires a destination flagged',
                '      # "Use for device-write commands" in the admin UI before anything',
                '      # actually subscribes.',
                '      - COMMANDS_ENABLED=true',
                '      # Default is 30s (src/commands/index.ts); widened for manual testing —',
                '      # checked against issuedAt regardless of expiresAt.',
                '      - COMMANDS_MAX_AGE_SECONDS=300',
