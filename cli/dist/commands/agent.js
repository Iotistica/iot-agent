...
exports.agentPullTargetState = agentPullTargetState;
 ...
 * iotctl agent pull
 */
async function agentPullTargetState() {
    try {
        const result = await (0, core_1.apiRequest)(`${core_1.DEVICE_API_V1}/sync/pull`, {
            method: 'POST',
        });
        core_1.logger.info('Target-state pull completed', {
        ...
        if (error instanceof core_1.CLIError)
            throw error;
        throw new core_1.CLIError('Failed to pull target state from cloud', 1, {
            error: error.message,
        });
    }
}
