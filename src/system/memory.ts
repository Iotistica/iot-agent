...
function updateBaselineIfStable(currentHeap: number, currentRSS: number): void {
	...
	// Don't attempt baseline updates too frequently (30 min minimum). Gated on the last
	// *attempt* (not just successful updates) so a persistently-rejected update doesn't
	// re-evaluate and re-log on every health-check tick until it finally succeeds.
	if (now - lastBaselineAttempt < BASELINE_UPDATE_INTERVAL_MS) {
		return;
	}
