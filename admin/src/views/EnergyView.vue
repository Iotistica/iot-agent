...
import type { EnergyRule, EnergyRecommendation, RecommendationStatus, RecommendationPublishSettings, Destination } from '@/types'
...
const rules = ref<EnergyRule[]>([])
const rulesLoading = ref(false)
const drawerOpen = ref(false)
const editing = ref<EnergyRule | null>(null)

const recommendations = ref<EnergyRecommendation[]>([])
const recsLoading = ref(false)

const ruleColumns: TableColumnType<EnergyRule>[] = [
...
const recColumns: TableColumnType<EnergyRecommendation>[] = [
...
async function loadRules() {
  ...
  try {
    rules.value = await energyApi.getRules()
  } catch (err: unknown) {
...
async function savePublishSettings() {
  ...
  try {
    publishSettings.value = await energyApi.updatePublishSettings(publishSettings.value)
...
function openEdit(row: EnergyRule) {
  editing.value = row
  drawerOpen.value = true
}

function confirmDelete(row: EnergyRule) {
  Modal.confirm({
...
async function setStatus(row: EnergyRecommendation, status: RecommendationStatus) {
  try {
...
<template>
  <AppLayout title="Energy">
    ...
    <EnergyRuleDrawer v-model:open="drawerOpen" :editing="editing" @saved="loadRules" />
  </AppLayout>
</template>
